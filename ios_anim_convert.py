...
import argparse, math, os, shutil, subprocess, sys, tempfile, uuid
...
def save_frames_to_pngs(frames: List[Image.Image], outdir: Path, base="frame") -> List[Path]:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        for px in size_steps:
            ...
            for f in frames:
                ...
                resized = f.resize((max(1, int(f.width*ratio)), max(1, int(f.height*ratio))), Image.LANCZOS)
                canvas = Image.new("RGBA", (px, px), (0, 0, 0, 0))
                canvas.paste(resized, ((px - resized.width)//2, (px - resized.height)//2), resized)
                sized.append(canvas)
            ...
            for colors in color_steps:
                ...
                if size_ok(out_apng):
                    print(f"[OK] Sticker APNG created at {out_apng} ({out_apng.stat().st_size/1024:.1f} KB, {px}px, {colors} colors)")
                    return
        # If we reach here we didn’t get under 500 KB
        print(f"[WARN] Could not compress < 500 KB. Output is {out_apng.stat().st_size/1024:.1f} KB. "
              f"Try fewer frames or smaller size.", file=sys.stderr)
...
def make_live_photo(infile: Path, out_base: Path, jpg_quality=92, try_tag=True):
    ...
    print(f"[OK] Live Photo components written:\n  Photo: {key_jpg}\n  Video: {video_mov}")
    ...
    if try_tag:
        try:
            ...
            asset_id, pvt_path = save_live_photo_pair_as_pvt(str(key_jpg), str(video_mov))
            print(f"[OK] Also wrote shareable package: {pvt_path}")
        except Exception as e:
            print("[INFO] Could not tag Live Photo automatically (likely not macOS or 'makelive' missing).", file=sys.stderr)
            print("       Files are ready. On a Mac, install 'makelive' to embed the required Asset ID.", file=sys.stderr)
            print(f"       Reason: {e}", file=sys.stderr)

def main():
    p = argparse.ArgumentParser(description="Convert animated GIF/APNG/WebP to iOS Sticker APNG and/or Live Photo pair.")
    ...
    p.add_argument("--live", type=Path, help="Output base path for Live Photo (no extension)")
    p.add_argument("--size", default="large", help="Sticker size: small|medium|large or pixel value (default: large)")
    p.add_argument("--no-tag", action="store_true", help="Skip Live Photo metadata tagging even if makelive is present")
    args = p.parse_args()
